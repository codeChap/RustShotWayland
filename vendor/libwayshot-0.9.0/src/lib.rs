...
// --- RustShot: deadline for every blocking_dispatch wait loop -----------------
//
// libwayshot waits for compositor replies with
//     while !state.some_flag { event_queue.blocking_dispatch(&mut state)?; }
// with no timeout. If the compositor never delivers the expected event — stale
// connection after a Hyprland restart, DPMS-off output, a dropped
// screencopy frame — `blocking_dispatch` keeps returning without setting the
// flag and the loop spins at 100% CPU forever. RustShot used to hit this via
// its `wayshot-one` timeout wrapper: each timed-out screenshot abandoned one
// such thread, and they accumulated (10 threads ≈ 1000% CPU after 3 days).
// The fix is to bound every one of those waits by a wall-clock deadline.
pub(crate) mod deadline {
    use crate::Error;
    use std::time::{Duration, Instant};

    /// Max time any single blocking_dispatch wait loop may run.
    /// Generous so slow-but-alive captures (multi-output, heavy load) still finish.
    pub(crate) const WAIT_BUDGET: Duration = Duration::from_secs(10);

    #[inline]
    pub(crate) fn guard(deadline: Instant, what: &str) -> crate::Result<()> {
        if Instant::now() > deadline {
            tracing::warn!(
                "wayshot {} wait exceeded {:?}; giving up",
                what,
                WAIT_BUDGET
            );
            return Err(Error::EventLoopTimeout);
        }
        Ok(())
    }
...
impl WayshotConnection {
    ...
    fn capture_output_frame_get_state_wlr(
        ...
        while !state.buffer_done.load(Ordering::SeqCst) {
            deadline::guard(deadline, "screencopy buffer_done")?;
    ...
    fn capture_output_frame_get_state_ext(
        ...
        while !state.session_done {
            deadline::guard(deadline, "image-copy output session")?;
            event_queue.blocking_dispatch(&mut state)?;
        }
        tracing::trace!(
            "Received compositor frame buffer formats: {:#?}",
    ...
    fn capture_output_frame_inner_dmabuf_ext<T: AsFd>(
        ...
        loop {
            ...
            deadline::guard(deadline, "frame copy")?;
            event_queue.blocking_dispatch(&mut state)?;
        }
    }
        ...
        for (_, frame_guard, output_info) in frames {
            ...
            .in_scope(|| -> Result<()> {
                ...
                while !state.configured_outputs.contains(&output_info.wl_output) {
                    deadline::guard(deadline, "layer surface configure")?;
    ...
    fn capture_toplevel_frame_get_state(
        ...
        let deadline = deadline::start();
        while !state.session_done {
            deadline::guard(deadline, "image-copy toplevel session")?;
            event_queue.blocking_dispatch(&mut state)?;
        }
