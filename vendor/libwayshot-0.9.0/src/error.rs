...
pub enum Error {
    ...
    Unsupported(String),
    /// A Wayland event loop (screencopy / image-copy capture, layer-shell
    /// configure, etc.) did not make progress within the wall-clock budget.
    /// Without this, such loops spin at 100% CPU forever.
    #[error("wayshot event loop timed out waiting for the compositor")]
    EventLoopTimeout,
    #[cfg(feature = "dmabuf")]
